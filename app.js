// Base de datos en localStorage
        ...
        // Eventos de búsqueda y filtro
            document.getElementById('buscarTrabajoInput').addEventListener('input', loadTrabajos);
            document.getElementById('filtrarEstadoSelect').addEventListener('change', loadTrabajos);
    ...
    const inputBusqueda = document.getElementById('buscarTrabajoInput');
    const filtroEstado = document.getElementById('filtrarEstadoSelect');
    ...
    const trabajosFiltrados = trabajosUsuario.filter(t => {
        const cliente = DB.clientes.find(c => c.id === t.cliente_id);
        const nombreCliente = cliente ? cliente.nombre.toLowerCase() : '';
        const clienteCoincide = nombreCliente.includes(textoBusqueda);
        const estadoCoincide = estadoSeleccionado === '' || t.estado === estadoSeleccionado;
        return clienteCoincide && estadoCoincide;
    });

    if (trabajosFiltrados.length === 0) {
        trabajosTableBody.innerHTML = `
            <tr><td colspan="7" class="text-center">No se encontraron trabajos</td></tr>
        `;
        return;
    }
